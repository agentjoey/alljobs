...
export function AssistantAnswer({ answer, stale = false, onUseTaskDraft }: { answer: ManagementAnswer; stale?: boolean; onUseTaskDraft?: (candidate: ManagementRecommendation) => void }) {
  return (
    <section className="assistant-output" aria-label="Companion output">
      ...
      {answer.recommendations.length > 0 && <div className="assistant-output__section"><h3>Recommendation</h3>{answer.recommendations.map((recommendation) => <div key={recommendation.id}><p>{recommendation.title}: {recommendation.rationale}</p>{!stale && recommendation.candidate_kind === "task" && <button className="btn" type="button" onClick={() => onUseTaskDraft?.(recommendation)}>Use as Task draft</button>}</div>)}</div>}
    </section>
  );
}
