...
- 实现负责人：KimiCode（`k3-256k`）；Codex 已完成独立验收与最终修复。
- P4 本地 implementation 已通过；P4-A、P4-B、P4-C 未通过。真实 Vault/Agent root、dry-run、publish 与 rollback 仍须对应 Human Gate。

**2026-09-17 implementation acceptance（base 4b373dd，final code d595e64）：**P4 本地实现已通过 Codex 最终验收。最后一轮补齐 approved plan → operation → marker → exact bytes 的完整绑定、Release/plan/Deployment exact authority、pre-lease 与 under-lock 双重 root/sentinel/authority revalidation（包含 realized crash-convergence）、rollback target bytes、marker-less unsafe directory fail-closed 与 canonical pointer fixture。最终门禁：152 files / 1373 tests PASS、typecheck PASS、lint 0 errors / 79 warnings、无 warning 的 production build、deploy invariants PASS、P4 E2E 11/11（fixture PostgreSQL + sentinel-owned 临时根）。单次 scoped final Review 的 1 Critical / 3 Important 已全部在 `7ae8402` 修复，fix-only re-review PASS；verifier 新发现的 realized-recovery TOCTOU 已在 `d595e64` 修复，independent re-verification PASS，绑定 evidence SHA `20e2a71`。未发生真实 root/provider/production DB/push/merge/deploy/release。P4-A/P4-B/P4-C 仍是关闭的 Human gates；本地实现验收不授权任何真实 dry-run、adapter enablement、publish 或 rollback。
...
- 数据库/对象树决策：既有 Neon `alljobs` Production branch 的 TLS Registry 与 private Object Storage；本地 Capture tree 保留为只读回滚源。N1 已创建 private `caphub-objects` bucket、`caphub` database、LOGIN `caphub_migrator`/`caphub_app` roles，并启用 transaction pooler；数据库及独立 storage-only credential 引用均只在 mode-`600` LaunchAgent。N2 已将一个不可变对象上传并独立读回哈希验证，且已记录 owner-only attestation。N3 已在确认的 source digest 条件下应用 checksum-bound migrations，并导入 2 条 Capture Registry metadata；post-import preflight 已匹配 Registry 与 N2 对象证明。
...
- N1/PA-B-N 已控制并完成真实 Neon bucket/database/roles/credential/config 写入；N2 immutable object transfer 与 N3 checksum migration/Registry import 已分别在显式授权下完成。IP allowlist 经 Human 决定不作为本单一 Control Host 部署的前置条件。PA-C 控制每次真实 provider request；PA-D 控制 push/merge/release/rebuild/reload/cutover。
