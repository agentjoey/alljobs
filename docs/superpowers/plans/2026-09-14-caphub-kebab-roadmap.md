...
- 数据库/对象树决策：既有 Neon `alljobs` Production branch 的 TLS Registry 与 private Object Storage；本地 Capture tree 保留为只读回滚源。N1 已创建 private `caphub-objects` bucket、`caphub` database、LOGIN `caphub_migrator`/`caphub_app` roles，并启用 transaction pooler；两条私有数据库 URL 已写入 mode-`600` LaunchAgent，尚未迁移或传输对象。
