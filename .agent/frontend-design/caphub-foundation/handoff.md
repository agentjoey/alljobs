...
P2 planning preflight is active under Linear `AGE-251`. The Human-authorized P2-A probes passed for MiniMax M3 image/strict-JSON and Kimi local-login K3-256K/no-tools; evidence is `.agent/caphub/p2-provider-probes.md`. Kimi API-key mode remains unproven because no credential/base-URL binding was clearly applicable; no trial request was sent. Stop before finalizing a dual-mode implementation plan until the Human Owner provides/identifies that binding or explicitly revises the P2 prerequisite to make local-login the only live-verified primary while retaining an unexercised fixture-only API contract. Do not enable Caphub, call another real provider, edit real production configuration, update LaunchAgents, restart production services, deploy, publish, push, merge, tag, release, switch traffic, or delete Caphub state.
